...
```
1. Catalog resolution       metric-paths, event/cause/problem/action + action-arg table (§1 + §1.1)
2. Name resolution + full type check  04-type-system.md §3–§7; binding env Γ
3. EffectInPurePosition     evidence rules must not emit Problem; decision rules must not infer Cause; no effectful/topology calls in pure expression positions (anchor predicates, window bounds) (ADGL0501)
4. Capability check         requires ⊆ catalog.capabilities                                       (§6)
5. Scope/target compat      rule.scope ⊑ target-scope, cause/problem валидны                      (§4, C3)
6. Temporal bound calc      time-окна вычислимы, MAX_LOOKBACK-инвариант                            (§3, C4)
7. Topology signature       topo-функции из каталога, arity, T3-возврат                            (§5, C10)
8. Rule-DAG acyclicity      нет циклов evidence→decision→evidence                                  (§2)
9. Exclusivity well-formed  mutually_exclusive ссылается на валидные Cause                         (§7, C5)
10. Bipartite isolation     evidence не emit Problem; decision не infer Cause                      (§8, C6)
11. DoS limits              окна, dedup, nesting, requires-count                                   (§9, C11)
12. Privacy annotations     pii-поля из каталога propagated в evidence                              (§10, C9)
→ Verified IR annotations   (§11–12)
```

The numbered pipeline order is normative; document § order differs.
...
### 8.1 EffectInPurePosition (ADGL0501)

`ADGL0501` фиксирует запрещённые формы в «чистых» позициях выражений:

- в `EvidenceRule` запрещён `emit Problem(...)`;
- в `DecisionRule` запрещён `infer Cause(...)`;
- в anchor-предикатах и выражениях границ окна (`time: ... in [a, b]`) запрещены
